...
description: Operate PlateOps across Slack, Linear, GitHub, and the deployed product. Use for intake triage, company orientation, product planning, ready engineering execution, review, release reporting, or onboarding.
---
...
engineers and QM operators. PlateOps has five
co-founders; product, operations, and sales teammates may own and refine OPS
work and product decisions, but they do not receive engineering or production
credentials by owning product work.
...
4. Confirm that engineering work is an accepted, human-owned Linear ENG work
   order, retains Mohamed or Abdullah as its accountable assignee, and satisfies
   the Definition of Ready: current priority, correct project and milestone,
   acceptance evidence, verification plan, native blockers, bounded non-goals,
   the source request or Slack thread, and a machine-checkable action policy.
   Readiness is not a workflow label. If the contract is incomplete, help refine
   the request in Product & Operations Triage and stop before code or
   infrastructure mutation. A complete work order dispatches without another
   founder approval ceremony.
...
Treat one immutable revision of one accepted, human-owned Linear ENG work order
as the contract for one QM run. Derive, verify, and restate:

- Linear issue ID, project, milestone, and one human owner;
- accepted revision, decision owner, and acceptance timestamp;
...
- whether opening a pull request, merging, preview delivery, or production
  delivery is allowed, the immutable policy source and digest, and the bounded
  retry limit.

Never put secret values in the work order. Use only scoped credential handles.
Resolve action policy from the accepted work-order revision, repository policy,
and environment mapping. The implementation agent cannot grant itself merge,
production, public-exposure, permission, secret, or destructive authority. A
policy-authorized exact-green merge may intentionally trigger the
repository-owned protected-main production workflow; it is not direct agent
production access.
...
and requires the revision to be accepted again.

Use the QM substates `ready`, `planning`, `executing`, `verifying`,
`human_review`, `delivery_pending`, `observing`, `completed`, `escalated`, and
`stopped`. Linear retains its normal human-facing workflow; do
...
or the next action is a direct production deploy, permission change,
destructive operation, or product-domain approval decision. Routine execution
may merge an exact green pull request when its action policy allows it, then
observe the protected-main delivery workflow; it must not bypass CI or directly
deploy production.
...
Engineering cycle without a recorded product decision and complete work order.

## Ready engineering execution

For accepted work, restate the outcome, scope, acceptance criteria, risks,
...
Before terminal write-back or a product/risk escalation, write the QM run/trace
and attempt IDs, plan summary, branch, pull request, commit, exact checks,
acceptance results, preview or Cloudflare deployment/trace identifiers when
relevant, remaining risk, and any requested decision to the Linear issue. A
stopped run still writes its terminal state and reason.
...
boundaries, reversible deployments, and human judgment for consequential
product, data, money, access, and destructive decisions. Full sandbox capability
is not permission to widen scope.
...
containment, route follow-up product work through the normal OPS-to-ENG
workflow.
...
user confirmation for consequential mutations. PlateOps AI/QM is the founders'
coding cockpit using each founder's private Codex subscription and QM's
swappable coding-runtime adapter, currently Codex. QM does not ship in the
PlateOps product and does not provide its inference credentials.
